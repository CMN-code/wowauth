...
- `CONFIG_SECRET` — the bearer secret from your `.env` (see `.env.example`)
- `BASE_URL` — where wowauth is reachable, e.g. `http://localhost:3000`
...
`openssl` — `apps.public_key` needs the *raw* 32-byte key, base64-encoded
...
curl -s -X POST "BASE_URL/apps" \
  -H "Authorization: Bearer CONFIG_SECRET" \
...
`redirect_url` is the *one* URL wowauth itself registers with the upstream
provider — fixed, and only known once `APP_ID` exists, hence the
placeholder in step 2. Update it now:

```sh
curl -s -X PATCH "BASE_URL/apps/$APP_ID" \
  -H "Authorization: Bearer CONFIG_SECRET" \
  -H "Content-Type: application/json" \
  -d '{"redirect_url": "'"BASE_URL/$APP_ID/oauth/callback"'"}' | jq .
```

Now go into the provider's developer console and set its "redirect URI" /
"callback URL" to that same value: `BASE_URL/$APP_ID/oauth/callback`.
...
echo "BASE_URL/$APP_ID/oauth/auth?redirect_uri=https://example.com/oauth-callback&state=$CALLER_STATE&code_challenge=$CHALLENGE&code_challenge_method=S256&account_hint=jane@example.com"
```

`account_hint` is optional — drop it entirely if you don't know which
account this is for ahead of time. Without it, every completed
authorization is treated as a distinct user (see step 6); with it, running
this flow again later for the same value reuses the same `USER_ID` instead
...
```sh
curl -s -X POST "BASE_URL/$APP_ID/oauth/token" \
...
```sh
curl -s "BASE_URL/apps/$APP_ID/users" -H "Authorization: Bearer CONFIG_SECRET" | jq .

USER_ID=$(curl -s "BASE_URL/apps/$APP_ID/users" -H "Authorization: Bearer CONFIG_SECRET" | jq -r '.[0].user_id')
...
curl -s "BASE_URL/apps/$APP_ID/users/$USER_ID/token" \
  -H "Authorization: Bearer CONFIG_SECRET" | jq .
```
...
token_info = subprocess.run(
    ["curl", "-s", f"{BASE_URL}/apps/{APP_ID}/users/{USER_ID}/token",
     "-H", f"Authorization: Bearer {CONFIG_SECRET}"],
...
```sh
curl -s "BASE_URL/apps/$APP_ID/users/$USER_ID/status" -H "Authorization: Bearer CONFIG_SECRET" | jq .
```

Revoke a user's access (deletes their stored token):

```sh
curl -s -X DELETE "BASE_URL/apps/$APP_ID/users/$USER_ID" -H "Authorization: Bearer CONFIG_SECRET"
```

Revoke a token directly, RFC 7009-style (used by whoever holds the token,
not necessarily an admin — no `CONFIG_SECRET` needed):

```sh
curl -s -X POST "BASE_URL/$APP_ID/oauth/revoke" --data-urlencode "token=$ACCESS_TOKEN"
```
...
The full, always-current API reference is served at `BASE_URL/docs/schema`
(OpenAPI JSON).
